  ...
  const ProfitRow = ({ label, profit, margin }: { label: string; profit: number; margin: number }) => (
    <div className="grid grid-cols-[minmax(90px,1fr)_minmax(80px,auto)_minmax(70px,auto)] gap-2 py-3 border-b dark:border-custom-dark-border last:border-b-0">
      ...
      <div className="text-right">
        <span className={`font-medium text-sm md:text-base ${profit >= 0 ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}`}>
          {profit.toFixed(2)}€
        </span>
      </div>
      <div className="text-right">
        <span className={`font-medium text-sm md:text-base ${margin >= 0 ? 'text-green-700 dark:text-green-300' : 'text-red-700 dark:text-red-300'}`}>
          {margin.toFixed(2)}%
        </span>
      </div>
    </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
        {/* EU Section */}
        <div className="bg-gray-100 dark:bg-custom-dark-bg rounded-lg p-4">
          <div className="mb-3">
            <h3 className="text-sm font-medium text-custom-text dark:text-custom-dark-text mb-1">With VAT (EU)</h3>
            <p className="text-xs text-gray-700 dark:text-custom-dark-text-secondary">Including VAT in calculations</p>
          </div>
          <div className="bg-white dark:bg-[rgb(35,35,35)] rounded-lg p-4 shadow-sm">
        ...
        <div className="bg-gray-100 dark:bg-custom-dark-bg rounded-lg p-4">
          <div className="mb-3">
            ...
            <p className="text-xs text-gray-700 dark:text-custom-dark-text-secondary">Excluding VAT from calculations</p>
          </div>
          <div className="bg-white dark:bg-[rgb(35,35,35)] rounded-lg p-4 shadow-sm">
            <div className="grid grid-cols-[minmax(90px,1fr)_minmax(80px,auto)_minmax(70px,auto)] gap-2 pb-2 mb-2 border-b dark:border-custom-dark-border">
              <div className="text-sm font-medium text-custom-text dark:text-custom-dark-text">Type</div>
